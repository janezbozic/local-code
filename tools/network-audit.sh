...
for port in 8080 8888 8890; do
  listeners="$("${lsof_bin}" -nP -iTCP:"${port}" -sTCP:LISTEN 2>/dev/null || true)"
  if [[ -n "${listeners}" ]]; then
    ...
    pid_lines="$("${lsof_bin}" -nP -t -iTCP:"${port}" -sTCP:LISTEN 2>/dev/null | sort -u)"
    ...
    if [[ "${port}" == 8080 ]]; then
      established="$("${lsof_bin}" -nP -a -p "${owner_pid}" -iTCP -sTCP:ESTABLISHED 2>/dev/null || true)"
...
for profile in $(sandbox_profiles); do
  case "${os}" in
    darwin)
      ...
      /usr/bin/sandbox-exec -f "${seatbelt}" /usr/bin/true || die "sandbox profile failed syntax probe: ${seatbelt}"
      ;;
    linux)
      "${root}/tools/sandbox/run.sh" --profile "${profile}" -- /bin/true || die "Linux sandbox failed for profile: ${profile}"
      ;;
  esac
done
